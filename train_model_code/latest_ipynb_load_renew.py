...
class SiameseGLDV2(Dataset):
    ...
    def plot(self, index: int):
        ...
        ----------
        index : int
            The number of index pair image in the data set
    ...
    def __getitem__(self, index):
        ...
        ----------
        index : int
         Which image in the dataset

        Returns
        -------
        transformed_anchor_im : query image
        transformed_pair_im : matching image
        target : target 1 means it's positive, otherwise netgative
        """
...
class ContrastiveLoss(nn.Module):
    ...
    # contractive loss function algorithm
...
# Define Siamese neutral network
# The convolutional layer of the neural network is
# the convolutional layer of Resnet50 and the last layer of the fully connected layer is removed
...
# margin : loss function parameter
margin = 1.
...
def load_model_continue_training(checkpoint_path, model):
    """
    load trained model from locality
    ...
    checkpoint_path : string
                      model saved address
    model ：nn.Module
            load the parameter of trained model
...
model = load_model_continue_training('model address', model)
...
def fit(train_loader, val_loader, model, loss_fn, optimizer, scheduler, n_epochs, cuda, log_interval, metrics=[],
        start_epoch=0):
    """
    I used the siamese neural network to train a landmark retrieval network loss function is contractive loss function.
    """

    # Start with the parameters to continue training
    for epoch in range(0, start_epoch):
        scheduler.step()

    # start_epoch start training
...
def train_epoch(train_loader, model, loss_fn, optimizer, cuda, log_interval, metrics, limit=1000000):
    ...
    Parameters
    ----------
    train_loader : Train dataset data loader
    model : Siamese neutral network
    loss_fn : Loss function
    optimizer : Optim.Adam
    cuda : GPU
    log_interval : Print Trainloss information every 500 batches
    metrics :
    limit : You can set the number of training images yourself
    ...
    # Check the database (limit, target, data) information
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # Optimizer parameters return to 0
        # Outputs : Forward propagation
        optimizer.zero_grad()
        outputs = model(*data)

        # Check whether the output data type is tuple or list.
        if type(outputs) not in (tuple, list):
            outputs = (outputs,)

        # for example outputs (2,2,2) target (1,0,1) loss_inputs += target (2,2,2,1,0,1)
    ...
    # total_loss calculates the average loss value
...
# parameter save epoch model_state_dict optimizer_state_dict loss model_2.pt
torch.save(
...
import pandas as pd
# Save train_loss and valid_loss to the csv file
